...
exports.updateProfileByUsername = async (req, res) => {
    try {
        let { bio, website, location, gender } = req.body;
        const { username } = req.params;

        const updatedData = { bio, location, gender };

        // 👉 Đảm bảo website là mảng trước khi lưu
        if (Array.isArray(website)) {
            // Lọc các liên kết không hợp lệ (chỉ là ví dụ đơn giản)
            updatedData.website = website.filter(
                (link) =>
                    typeof link === 'string' && link.trim().startsWith('http')
            );
        }
