    ...
    import { getPostDetails, toggleLike, addComment, toggleCommentLike} from "~/api/post";
    import { Heart, MessageCircle, Send, ChevronLeft, ChevronRight, X, MoreHorizontal  } from "lucide-react";
    ...
    export default function PostDetailPage({ isModal = false }) {
        ...
        const [likesCommentCount, setLikesCommentCount] = useState(postDetails?.comments.likes?.length || 0);
        ...
        const handleCommentLike = async (commentId) => {
            try {
                const res = await toggleCommentLike(postDetails.post._id, commentId);
        ...
        const handleAddComment = async () => {
            ...
            const fullComment = {
                ...
                isLiked: false, // Mặc định bình luận mới chưa được like
                likesCount: 0, // Đặt lại số lượng likes ban đầu
            };
        ...
        useEffect(() => {
            const fetchPostDetails = async () => {
                try {
                    ...
                    // Cập nhật postDetails với likesCount và isLiked của bình luận
                    const updatedComments = comments.map(comment => ({
                        ...comment,
                        isLiked: comment.likes.includes(user.id), // Kiểm tra xem user có like bình luận này không
                        likesCount: comment.likes.length, // Số lượng likes của bình luận
                    }));
        
                    // Cập nhật lại postDetails state
                    setPostDetails({
                        ...response.data,
                        comments: updatedComments
                    });
                        ...
                        .map((c, idx) => (
                            ...
                            >
                                ...
                                <div className="break-words break-all whitespace-pre-wrap leading-snug">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        {c.likesCommentCount >= 0 && 
                                            <p className="text-xs text-gray-400 px-2">{c.likesCommentCount} lượt thích</p>
                                        }
                                ...
                                <Heart
                                    ...
                                    c?.isLikedComment ? "fill-current text-red-500" : "text-gray-500"
                                    }`}
