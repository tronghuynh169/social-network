...
const LikesModal = ({ postId, currentUserId, onClose }) => {
        ...
        const mergedLikesUsers = await Promise.all(
          postData.likes.map(async (likeUser) => {
            ...
            if (followingMap.has(likeUser._id)) {
              ...
              try {
                profile = await getProfileByUserId(likeUser._id);
              } catch (error) {
  ...
  const handleFollowToggle = async (targetProfileId) => {
    ...
    try {
      ...
      const targetUser = likesUsers.find(user => user._id === targetProfileId);
      ...
      setFollowingLoading(prev => ({ ...prev, [targetProfileId]: false }));
    }
  };
          ...
          <div className="overflow-y-auto max-h-[65vh]">
            {loading ? (
              ...
              likesUsers.map(user => {
                ...
                const isFollowing = followingList.has(user.userId);

                return (
                  <div key={user.userId} className="flex items-center justify-between px-4 py-3 hover:bg-neutral-800 transition">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        {user.fullName && <p className="text-[var(--text-primary-color)]-400 text-sm">{user.fullName}</p>}
                      </div>
                    </div>
